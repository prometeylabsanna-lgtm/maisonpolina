...
MSG_NAME_DIGITS = _("Имя не должно содержать цифр.")
MSG_PHONE = _(
    "Номер телефона не может содержать буквы и должен содержать от 7 до 14 цифр."
)
MSG_MESSAGE_MIN = _("Текст отзыва должен содержать минимум 2 символа.")
...
def validate_phone(value: str) -> str:
    ...
    if (
        not contact
        or not _PHONE_ALLOWED.fullmatch(contact)
        or digits < _MIN_PHONE_DIGITS
        or digits > _MAX_PHONE_DIGITS
    ):
